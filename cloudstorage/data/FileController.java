    ...
    synchronized public void upload(FileData fileData)
    {
        ...
        for(int i = 0; i < blocksCreated.size(); i++)
        {
            ...
            for(int j = 0; j < packetsCreated.size(); j++)
            {
                ...
                //ui.appendToLog(String.format("NUM_PACKETS + filecontroller: %d OUT OF %d", (packetsCreated.get(j)[1] + 1),
                //    packetsCreated.size()));
                ui.changeSyncStatus("In Progress");

                ui.progress1.setMaximum(packetsCreated.size());
                ui.progress1.setValue(packetsCreated.get(j)[1] + 1);
                ui.progress1.setString(String.format("Packets transmitted (%d/%d)", (packetsCreated.get(j)[1] + 1),
                packetsCreated.size()));

                ui.progress2.setMaximum(blocksCreated.size());
                ui.progress2.setValue(i + 1);
                ui.progress2.setString(String.format("Blocks transmitted (%d/%d)", (i + 1), blocksCreated.size()));
